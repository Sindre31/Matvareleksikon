...
(function () {
  ...
  var state = {
    ...
    // Næringsinnhold: which portion the table is scaled to (0 = per 100 g) and
    // whether the vitamins and minerals are unfolded.
    nutriPortion: 0, nutriAll: false,
  ...
  function route() {
    ...
    if (r.view === 'gruppe') {
      ...
      // not to the shopper.
      if (state.groupKey !== r.groupKey) {
        state.groupStore = 'Alle'; state.groupSize = 'alle';
        // The portion chips are the matched food's own ("1 glass", "1 skive"),
        // so the choice cannot survive into another product.
        state.nutriPortion = 0;
      }
  ...
  // Mattilsynet publishes the whole food composition table as open data, and
  // "hvor mye protein er det i denne" is the second question a shopper asks
  // after "hva koster den". The catch is that the two datasets have nothing in
  // common: the leksikon holds ~37 500 branded packs ("Kyllingfilet Naturell
  // 800g Prior"), Matvaretabellen holds 2 121 generic foods ("Kylling, filet,
  // uten skinn, rå"), and no id, EAN or key joins them. The only bridge is the
  // name, so everything below is about being *right or silent* — a plausible
  // wrong table is worse than none, because a shopper has no way to tell.
  //
  // Three things keep it honest:
  //   1. the head gate (matchNutrition) — the first word of a Matvaretabellen
  //      name is what the food IS, and the product has to carry that word;
  //   2. a score floor, calibrated against the live catalogue (see MIN_SCORE);
  //   3. the screen NAMES the food it matched and links to its page on
  //      matvaretabellen.no, so the guess is always checkable.
  // Roughly a fifth of the multi-store groups clear all three. The rest —
  // branded snacks, drinks, non-food — simply get no section, which is the
  // correct answer for a table that does not contain them.
  //
  // nutrition.json is generated by tools/build-nutrition.mjs and committed;
  // it is fetched once, on the first product page a visit opens, and the
  // service worker caches it from there like any other static asset.
  var NUTRITION = null;          // the index built from nutrition.json
  var NUTRITION_STATE = 'idle';  // idle | loading | ready | failed
  var NUTRITION_HIT = {};        // group key -> match | null, memoised

  // Below this the match is a coincidence. Calibrated on the 5 075 groups at
  // least two chains carry: at 12 the accepted matches are overwhelmingly the
  // right food, and the band just underneath is where "Jordan mango mint
  // tannkrem" wants to be a mango and a box of beer wants to be dessert
  // topping. Raising it costs real matches ("gulost lett", "olje tunfisk");
  // lowering it buys nonsense.
  var MIN_SCORE = 12;

  function wantNutrition() {
    if (NUTRITION_STATE !== 'idle') return;
    NUTRITION_STATE = 'loading';
    fetch('/nutrition.json')
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (data) {
        NUTRITION = data ? buildNutrition(data) : null;
        NUTRITION_STATE = NUTRITION ? 'ready' : 'failed';
        render();
      })
      .catch(function () { NUTRITION_STATE = 'failed'; render(); });
  }

  // Words that say nothing about what a food IS — producers (Matvaretabellen
  // names some foods after the brand that was analysed), prepositions, and the
  // table's own "uspesifisert". They neither anchor a match nor count against
  // one.
  var NUT_NEUTRAL = {
    tine: 1, q: 1, meieriene: 1, yoplait: 1, gilde: 1, prior: 1, nordfjord: 1, synnove: 1,
    mills: 1, stabburet: 1, freia: 1, kavli: 1, jacobs: 1, coop: 1, rema: 1, kiwi: 1, meny: 1,
    nugatti: 1, hapa: 1, semper: 1, coffeemate: 1, kellogg: 1, nestle: 1, toro: 1, fjordland: 1,
    uspesifisert: 1, fett: 1, ca: 1, kjopt: 1, og: 1, med: 1, uten: 1, ved: 1, ell: 1, eller: 1,
    pr: 1, per: 1, type: 1, sort: 1, sorter: 1, gjennomsnitt: 1, alle: 1, ulike: 1
  };
  // How the food was PREPARED. A preparation the product name does not mention
  // is the wrong food — someone buying a fillet wants the raw numbers, not the
  // pan-fried ones — so an unmatched one of these costs three times an
  // ordinary unmatched qualifier. ("kjøpt" is deliberately absent: everything
  // in this leksikon is bought.)
  var NUT_PREP = {
    stekt: 1, ovnsstekt: 1, kokt: 1, grillet: 1, fritert: 1, forfritert: 1, bakt: 1,
    hjemmelaget: 1, hjemmebakt: 1, tilberedt: 1, traktet: 1, gratinert: 1, dampet: 1,
    posjert: 1, gravet: 1, rokt: 1, syltet: 1, saltet: 1, torket: 1, torr: 1,
    hermetisk: 1, fryst: 1, avrent: 1, oppvarmet: 1, pulver: 1, spiseklar: 1, konsentrert: 1
  };

  // Norwegian plurals and definite forms, so "bananer" and "banan" are one
  // word. Only stripped when what is left is still a word (4+ letters): "ris"
  // must not become "r". Memoised — matchNutrition runs it a few hundred
  // thousand times per product page.
  var NUT_STEM = {};
  function nutStem(w) {
    if (NUT_STEM[w] != null) return NUT_STEM[w];
    var m = w.match(/^(.{4,}?)(ene|ane|erne|ers|er|en|et|ar|a|e)$/);
    return (NUT_STEM[w] = m ? m[1] : w);
  }
  // Norwegian inflects a word's tail and nothing else: fin/fint/fine,
  // kake/kaker, potet/poteter. Two words of 4+ letters that agree for their
  // first three letters and differ only by inflection letters are the same
  // word — a short enough leash that "kjøtt" and "kjøttdeig" stay different.
  var NUT_INFL = /^[aenrt]{0,2}$/;
  function nutInflection(a, b) {
    if (a.length < 4 || b.length < 4) return false;
    var i = 0;
    while (i < a.length && i < b.length && a.charAt(i) === b.charAt(i)) i++;
    return i >= 3 && NUT_INFL.test(a.slice(i)) && NUT_INFL.test(b.slice(i));
  }
  // Does product word `p` carry food word `t`? Higher is a closer fit. The
  // compound rules are the same shape as the ones POPULAR matches categories
  // with, and for the same reason: Norwegian puts the meaning in the tail of a
  // compound, so "lettmelk" is milk and "melkesjokolade" is not.
  function nutCarries(p, t) {
    if (p === t) return 3;
    if (nutStem(p) === nutStem(t)) return 2.8;
    if (nutInflection(p, t)) return 2.6;
    if (t.length >= 4 && p.length > t.length && p.slice(-t.length) === t) return 2;
    if (t.length >= 5 && p.indexOf(t) >= 0) return 1;
    return 0;
  }
  function nutCarriesAny(P, t) {
    var best = 0;
    for (var i = 0; i < P.length; i++) { var s = nutCarries(P[i], t); if (s > best) best = s; }
    return best;
  }

  // Turn the payload into what the matcher and the table need: every food's
  // name split into the head segment and its qualifiers, folded the same way
  // group keys are folded.
  function buildNutrition(data) {
    if (!data || !Array.isArray(data.foods) || !Array.isArray(data.nutrients)) return null;
    var keep = function (t) { return t.length >= 3 && !NUT_NEUTRAL[t]; };
    var foods = data.foods.map(function (f) {
      var segs = String(f[1]).split(',');
      var head = foldName(segs[0]).split(' ').filter(keep);
      var rest = foldName(segs.slice(1).join(' ')).split(' ').filter(keep);
      return {
        id: f[0], name: String(f[1]), groupId: f[2], slug: f[3],
        kcal: f[4], kJ: f[5], values: f[6] || [], portions: f[8] || [],
        head: head, rest: rest, all: head.concat(rest),
        kw: (f[7] || []).map(foldName).filter(function (t) { return t.length >= 3; })
      };
    });
    return {
      foods: foods, nutrients: data.nutrients, foodGroups: data.foodGroups || {},
      source: data.source || '', edition: data.edition || '', retrieved: data.retrieved || ''
    };
  }

  // Score one food against the product's words, or null if it isn't in the
  // running at all.
  function scoreNutrition(P, f) {
    // 1 — the head gate. The first word of a Matvaretabellen name is what the
    // food is ("Kjøttdeig, storfe, …", "Brød, grovt, …"); everything after the
    // first comma only qualifies it. A product that doesn't carry that word is
    // not that food, however many qualifiers happen to line up.
    var head = f.head[0];
    if (!head) return null;
    var hit = 0, i;
    for (i = 0; i < P.length; i++) {
      var p = P[i], s = nutCarries(p, head);
      // "kyllingfilet" ← "Kylling, filet": here the head is the FIRST half of
      // the compound, which counts only when the second half is also one of
      // this food's own words. Without that, "melkesjokolade" matches "Melk".
      if (s < 2.5 && head.length >= 4 && p.length > head.length && p.lastIndexOf(head, 0) === 0) {
        var tail = p.slice(head.length).replace(/^e/, '');
        if (tail.length >= 3 && f.all.some(function (t) { return nutCarries(tail, t) >= 2; })) s = 2.5;
      }
      if (s > hit) hit = s;
    }
    if (!hit) return null;

    // Weighted by how much word was matched — "kjøttdeig" landing exactly is a
    // far stronger claim than "laks" landing exactly — up to a cap, past which
    // the length says more about Norwegian compounding than about the match.
    var score = hit * 4 + Math.min(head.length, 8) * hit * 0.15;

    // 2 — the rest of the name. A hit is worth more than a miss costs, or the
    // table's most precisely described foods could never win. A miss in the
    // head segment costs double: "Fløtegratinerte poteter" is not poteter.
    for (i = 1; i < f.head.length; i++) {
      var hs = nutCarriesAny(P, f.head[i]);
      score += hs ? hs * 1.2 : -2.0;
    }
    for (i = 0; i < f.rest.length; i++) {
      var t = f.rest[i], rs = nutCarriesAny(P, t);
      score += rs ? rs * 1.2 : (NUT_PREP[t] ? -3.0 : -1.0);
    }

    // 3 — and what the product says that the food does not. Half of it has to
    // land: Matvaretabellen has a mango and "Jordan mango mint tannkrem" says
    // "mango", but a toothpaste is not a mango. Keywords (the table's own
    // synonyms — "kaffi", "skaldyr") count here, never at the head gate.
    var said = 0, explained = 0;
    for (i = 0; i < P.length; i++) {
      var w = P[i];
      if (w.length < 3 || NUT_NEUTRAL[w]) continue;
      said++;
      var best = 0, j;
      for (j = 0; j < f.all.length; j++) best = Math.max(best, nutCarries(w, f.all[j]));
      for (j = 0; j < f.kw.length; j++) best = Math.max(best, nutCarries(w, f.kw[j]));
      if (best) explained++; else score -= 1.2;
    }
    if (said && explained / said < 0.5) return null;

    // 4 — tie-breaks. Raw over cooked, generic over branded, bought over
    // home-made: a group key names a product, not a preparation or a producer,
    // so the entry that assumes least is the honest one. Then the shortest
    // name — the least specific food that still fits.
    if (/, rå$/.test(f.name)) score += 1.0;
    if (/uspesifisert/i.test(f.name)) score += 0.8;
    if (/, kjøpt$/.test(f.name)) score += 0.5;
    score -= f.all.length * 0.05;
  ...
  // change (a size chip, a sort, the catalogue refresh landing).
  function nutritionFor(key) {
    if (NUTRITION_STATE !== 'ready') return null;
    if (!(key in NUTRITION_HIT)) NUTRITION_HIT[key] = matchNutrition(NUTRITION, key);
    return NUTRITION_HIT[key];
  }

  // A number the way Matvaretabellen prints it: its own decimal precision,
  // Norwegian comma, and "–" for a nutrient that was never analysed for this
  // food (which is not the same as zero, and must not read as zero).
  ...
  function renderGroup() {
    ...
    return h('section', { 'data-screen-label': 'Produktgruppe' }, [head, table, histSection, nutritionSection(g)]);
  }

  // ── "03 · Næringsinnhold" ────────────────────────────────────────────────
  // Shown only when matchNutrition is confident (see MIN_SCORE), and always
  // with the matched food named and linked: the numbers describe a generic
  // food from Matvaretabellen, not the declaration on this particular pack,
  // and a shopper has to be able to see which food that is. Absent entirely
  // for the products the table doesn't cover — most of the leksikon.
  function nutritionSection(g) {
    wantNutrition();
    if (NUTRITION_STATE === 'failed') return null;
    var section = function (body) {
      return h('div', { style: 'margin-top: 40px;' }, [
        h('span', { style: KICKER, text: '03 · Næringsinnhold' }),
        h('hr', { style: RULE })
      ].concat(body));
    };
    if (NUTRITION_STATE !== 'ready') {
      return section([h('p', { style: 'font-size: 15px; color: ' + MUTED70 + ';', text: 'Laster næringsinnhold …' })]);
    }
    var hit = nutritionFor(g.key);
    if (!hit) return null;
    var f = hit.food;

    // Per 100 g, or per one of the table's own household portions ("1 glass
    // (190 g)"). Nothing is scaled to the PACK: a litre of milk is not a
    // kilo of milk, and guessing a density to make it look precise would be
    // inventing a number.
    var portions = [{ label: 'Per 100 g', grams: 100 }].concat(f.portions.map(function (p) {
      return { label: '1 ' + p[0] + ' (' + p[1] + ' g)', grams: p[1] };
    }));
    var pi = Math.min(Math.max(state.nutriPortion | 0, 0), portions.length - 1);
    var per = portions[pi];
    var scale = per.grams / 100;

    var row = function (label, value, unit, opts) {
      opts = opts || {};
      return h('div', {
        style: 'display: flex; justify-content: space-between; align-items: baseline; gap: 16px; padding: 7px 20px;'
          + (opts.rule ? ' border-top: 1px solid color-mix(in srgb, var(--color-text) 8%, transparent);' : '')
      }, [
        h('span', {
          style: 'font-size: ' + (opts.strong ? '15px' : '14px') + ';'
            + (opts.strong ? ' font-weight: 600;' : ' color: ' + MUTED78 + ';')
            + (opts.indent ? ' padding-left: 18px;' : ''),
          text: opts.indent ? 'herav ' + label.toLowerCase() : label
        }),
        h('span', {
          style: "font-size: " + (opts.strong ? '15px' : '14px') + "; font-feature-settings: 'tnum' 1; white-space: nowrap;"
            + (opts.strong ? ' font-weight: 600;' : ''),
          text: value + (value === '–' || !unit ? '' : ' ' + unit)
        })
      ]);
    };
    var valueRow = function (n, i, first) {
      var v = f.values[i];
      return row(n.name, nutNum(v == null ? null : v * scale, n.dec), n.unit,
        { indent: n.indent, rule: first && !n.indent });
    };

    // kcal and kJ on one line, kcal first: it is the number people read, and
    // the kJ is the one a package is legally obliged to print beside it.
    var kcal = nutNum(f.kcal == null ? null : f.kcal * scale, 0);
    var kJ = nutNum(f.kJ == null ? null : f.kJ * scale, 0);
    var rows = [row('Energi', kcal === '–' ? '–' : kcal + ' kcal (' + kJ + ' kJ)', '', { strong: true })];
    NUTRITION.nutrients.forEach(function (n, i) {
      if (n.decl) rows.push(valueRow(n, i, rows.length === 1));
    });
    if (state.nutriAll) {
      var sections = {};
      NUTRITION.nutrients.forEach(function (n, i) {
        if (!n.section) return;
        (sections[n.section] || (sections[n.section] = [])).push(valueRow(n, i, false));
      });
      Object.keys(sections).forEach(function (name) {
        rows.push(h('div', {
          style: 'padding: 14px 20px 6px; border-top: 1px solid color-mix(in srgb, var(--color-text) 8%, transparent);'
        }, [h('span', { style: KICKER + ' margin-bottom: 0;', text: name })]));
        rows.push.apply(rows, sections[name]);
      });
    }

    var chip = function (label, on, onClick) {
      return h('button', {
        type: 'button', cls: 'btn ' + (on ? 'btn-primary' : 'btn-ghost'), 'aria-pressed': on ? 'true' : 'false',
        style: 'min-height: var(--tap-row, 32px); padding: 3px 12px; font-size: 12px; letter-spacing: 0.06em;',
        onClick: onClick, text: label
      });
    };
    var controls = portions.length > 1 ? h('div', {
      role: 'group', 'aria-label': 'Vis næringsinnhold per', style: 'display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 18px;'
    }, portions.map(function (p, i) {
      return chip(p.label, i === pi, function () { setState({ nutriPortion: i }); });
    })) : null;

    var groupName = NUTRITION.foodGroups[f.groupId] || '';
    return section([
      h('p', { style: 'margin: 0 0 18px; font-size: 15px; line-height: 22px; color: ' + MUTED78 + ';' }, [
        'Nærmeste oppslag i Matvaretabellen er ',
        h('a', {
          href: 'https://www.matvaretabellen.no/' + f.slug + '/', target: '_blank', rel: 'noopener',
          style: 'font-weight: 600;', text: f.name
        }),
        groupName ? ' (' + groupName.toLowerCase() + ')' : '',
        '. Det er en generisk matvare — ikke varedeklarasjonen på akkurat denne pakningen — så tallene ',
        'sier hva slags mat dette er, ikke hva produsenten har målt.'
      ]),
      controls,
      h('div', { cls: 'blueprint', style: 'padding: 0;' }, corners().concat(rows)),
      h('div', { style: 'display: flex; flex-wrap: wrap; gap: 10px 16px; align-items: center; margin-top: 16px;' }, [
        h('button', {
          type: 'button', cls: 'btn btn-secondary', 'aria-expanded': state.nutriAll ? 'true' : 'false',
          style: 'min-height: var(--tap-row, 34px); padding: 4px 14px; font-size: 12px; letter-spacing: 0.06em; text-transform: uppercase; font-weight: 600;',
          onClick: function () { setState({ nutriAll: !state.nutriAll }); },
          text: state.nutriAll ? 'Vis bare hovedtallene' : 'Vis vitaminer og mineraler'
        })
      ]),
      h('p', { style: 'margin: 14px 0 0; font-size: 13px; line-height: 19px; color: ' + MUTED60 + ';' }, [
        'Kilde: ' + NUTRITION.source + '. ',
        '«–» betyr at næringsstoffet ikke er analysert for denne matvaren, som ikke er det samme som null.'
      ])
    ]);
  }
  ...
  function renderAbout() {
    ...
    var sources = aboutSection('Kilder', '02', [
      ...
      // Mattilsynet requires a visible source line wherever the table's values
      // are shown. The product page prints one under every table; this is the
      // same credit in the place a reader looks for it.
      h('p', { style: 'margin: 14px 0 0; font-size: 14px; line-height: 22px; color: ' + MUTED60 + '; max-width: 68ch;' }, [
        'Næringsinnholdet på produktsidene kommer fra ',
        h('a', { href: 'https://www.matvaretabellen.no/', target: '_blank', rel: 'noopener', text: 'Matvaretabellen' }),
        ' (Mattilsynet), som er åpne data. Vi kobler varen i butikken til den matvaren i tabellen som ligner mest, og skriver alltid hvilken det er — tallene gjelder en generisk matvare, ikke varedeklarasjonen på pakningen. Stemmer ikke koblingen, si fra med ⚠-knappen.'
      ]),
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
      ...
      buildNutrition: buildNutrition, matchNutrition: matchNutrition, nutNum: nutNum,
      NUTRITION_MIN_SCORE: MIN_SCORE
    };
  }
})();
