...
-- Real Rema 1000 shelf prices straight from Rema's own catalogue API (keyless
-- api.digital.rema1000.no), 40 min after the offers job. Rema is a single chain
-- → 'rema'. Complements Kassalapp so Rema no longer relies on a third-party index.
select cron.schedule(
  'ml-ingest-rema-weekly',
  '40 4 * * 1',
  $cmd$
  select net.http_post(
    url := 'https://jiaxeedguivvhixychcg.supabase.co/functions/v1/ml-ingest-rema',
    headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer <SUPABASE_ANON_KEY>'),
    body := '{}'::jsonb,
    timeout_milliseconds := 170000
  );
