...
Deno.serve(async (req: Request) => {
  ...
  return json({ mode: bulk ? "bulk" : "search", productsFound: rows.length, inserted, historyInserted, lastPage, hitEnd, nextPage: bulk && !hitEnd ? lastPage + 1 : null, elapsedMs: Date.now() - start });
});
