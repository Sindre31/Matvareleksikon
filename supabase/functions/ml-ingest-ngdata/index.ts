// Supabase Edge Function: ml-ingest-ngdata
// -----------------------------------------------------------------------------
// Real Meny shelf prices via NorgesGruppen's public ngdata API (keyless), the
// same source `billigkurv` uses for Meny/Spar. We ingest Meny (chainId 1300)
// into ml_offers (source='ngdata') with the shared group_key scheme, plus
// price history. Meny maps to Prisboka's existing 'meny' store; Spar/Kiwi/Rema
// have no per-store assortment here so we stick to Meny.
...
const API = "https://platform-rest-prod.ngdata.no";
const MENY = { chainId: "1300", gln: "7080000002153", store: "meny" };
const PAGE_SIZE = 100;
...
function rowFrom(hit: any): any | null {
  const price = hit?.pricePerUnit;
  if (typeof price !== "number" || !isFinite(price) || price <= MIN_PRICE_NOK || !hit?.title) return null;
  const name = hit.subtitle ? `${hit.title} ${hit.subtitle}`.trim() : hit.title;
  const isOffer = hit?.isOffer === true;
...
Deno.serve(async (_req: Request) => {
  ...
  for (const term of TERMS) {
    let hits: any[] = [];
    try {
      const res = await fetch(`${API}/api/products/${MENY.chainId}/${MENY.gln}/?search=${encodeURIComponent(term)}&fieldset=maximal&page_size=${PAGE_SIZE}`, { headers: { Accept: "application/json", "User-Agent": UA } });
      if (!res.ok) { console.error("ngdata", res.status, term); continue; }
      const j = await res.json(); hits = j?.hits ?? [];
    } catch (e) { console.error("ngdata fetch", term, String(e)); continue; }
    for (const hit of hits) { const r = rowFrom(hit); if (!r) continue; const prev = byKey.get(r.external_id); if (prev && Number(prev.price) <= r.price) continue; byKey.set(r.external_id, r); }
    await sleep(100);
  }
  const rows = [...byKey.values()];

  await fetch(`${SB_URL}/rest/v1/ml_offers?source=eq.ngdata`, { method: "DELETE", headers: sbHeaders });
  let inserted = 0;
